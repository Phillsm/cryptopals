...
(*
#r @"C:\Users\Phill\Dropbox\Programming\crypto\matasano\Matasano Crypto Challenges\packages\FSharp.Data.2.0.14\lib\net40\FSharp.Data.dll"
#load @"C:\Users\Phill\Dropbox\Programming\crypto\matasano\Matasano Crypto Challenges\Matasano Crypto Challenges\Convertions.fs"
#load @"C:\Users\Phill\Dropbox\Programming\crypto\matasano\Matasano Crypto Challenges\Matasano Crypto Challenges\MyCollectionLib.fs"
*)
open System
...
///////////////////////////////////////////
let chal10input = Http.RequestString("""http://cryptopals.com/static/challenge-data/10.txt""") |> Base64.create |> Base64.toBytes
...
let decryptAesEcbNoPadding (key : byte[]) (input : byte[])  = 
 //   printfn "Length of input is %i: %s" input.Length (Array.toString input)
    let aes = AesManaged.Create()
    aes.Key <- key 
    aes.Mode <- CipherMode.ECB
    aes.BlockSize <- 128
    ...
    use msDecrypt = new MemoryStream(input)
    use csDecrypt = new CryptoStream(msDecrypt,decryptor,CryptoStreamMode.Read)  
    use brDecrypt = new BinaryReader(csDecrypt)
    brDecrypt.ReadBytes(input.Length)


let myCBCencryptor (key : byte[]) (IV : byte[]) (input : byte[]) = 
...
let myCBCdecryptor (key : byte[]) (input : byte[]) =
    let Blocked = Array.chunksOf 16 (input)
    [| for x = 1 to (Blocked.Length - 1) do
        let decrypt = decryptAesEcbNoPadding key Blocked.[x]
        let plaintext = xorArrays decrypt Blocked.[x-1]
        yield plaintext |]
    |> Array.concat

let testcbc = 
    myCBCencryptor ("YELLOW SUBMARINE" |> String.toBytes) [|for x in 1..16 -> 0uy|] ("TESTTESTTEST THIS SHOULD BE MORE THAN THAN 16" |> String.toBytes)
    |> myCBCdecryptor ("YELLOW SUBMARINE" |> String.toBytes)
    |> Array.toString

let chal10output = 
    Array.append [|for x in 1..16 -> 0uy|] chal10input
    |> myCBCdecryptor ("YELLOW SUBMARINE" |> String.toBytes) 
    |> Array.toString
...
let getRandBytes(n : int) = 
    [|for x in 1..n -> (byte) (random.Next(256)) |]

let Gibberfy (input : byte[]) = 
    let preRand = getRandBytes(random.Next(5,11))
    let postRand = getRandBytes(random.Next(5,11))
    let randIV = getRandBytes(random.Next(16))
    let randKey = getRandBytes(random.Next(16))
    let encrypted = 
        if random.Next(2) = 0
        then  encryptAesEcbNoPadding key (autopad input)
        else myCBCencryptor randKey randIV input
    Array.append preRand (Array.append encrypted postRand)

let GibberfySpecify (input : byte[]) (encryptionMode : Encryptionmode) = 
    let preRand = getRandBytes(random.Next(5,11))
    let postRand = getRandBytes(random.Next(5,11))
    let randIV = getRandBytes(random.Next(16))
    let randKey = getRandBytes(random.Next(16))
    let encrypted = 
        match encryptionMode with
        | ECB ->  encryptAesEcbNoPadding key (autopad input)
        | CBC ->  myCBCencryptor randKey randIV input
    Array.append preRand (Array.append encrypted postRand)


let Chal11Solution = 
    let testtext = chal10input
    [for x in 1..20 do
        ]
[<EntryPoint>]
let main argv = 
